...
def plot_mobility(agg: Aggregate, path: Path | None, show: bool = False, footer: str | None = None) -> None:
    ...
    for ax, (key, unit, fmt) in zip(axes, PANELS):
        ...
        for k in active:
            ...
            if key == "cum_pax" and len(active) <= MAX_END_LABELS and np.isfinite(m[-1]):
    ...
    handles = [
        Patch(facecolor=TYPE_COLORS[k], label=f"{cfg.specs[k].name} ({_rate_label(cfg.rates[k])})") for k in active
    ] + [Patch(facecolor=RED_PHASE, alpha=0.2, label="semáforo en rojo")]
    ...
    fig.text(
        0.075, fy(0.958),
        f"Tramo {cfg.length:g} m, {cfg.lanes} carril(es) · rojo {cfg.red:g} s / verde {cfg.green:g} s · "
    ...
    if len(active) <= MAX_END_LABELS:
        longest = max(longest, *(len(f"{cfg.specs[k].name} 0,000") for k in active))
...
def plot_passenger_distribution(agg: Aggregate, path: Path, footer: str | None = None) -> None:
    ...
    fig.text(
        0.075, 1 - 0.58 / fh,
        f"Todos los vehículos que llegaron en {_duration(cfg.sim_seconds)} simulados, sumando "
